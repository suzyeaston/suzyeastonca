...
test('attention badges and cards have intrinsic desktop and mobile bounds', () => {
  ...
  assert.match(css, /\.lo-attention-services__item\s*{[^}]*display: grid[^}]*width: 100%/s);
  assert.match(css, /\.lo-service-card\s*{[^}]*width: 100%/s);
  assert.match(css, /\.lo-service-card__diagnostic\s*{[^}]*flex: 1[^}]*background-color: transparent[^}]*Inter/s);
  assert.match(css, /\.lo-service-card__footer p\s*{[^}]*background-color: transparent[^}]*Inter/s);
});
...
test('0.4.8 release metadata is canonical', () => {
  ...
  assert.match(plugin, /Version: 0\.4\.8/);
  assert.match(plugin, /LOUSY_OUTAGES_VERSION', '0\.4\.8'/);
  assert.match(readme, /## 0\.4\.8/);
  assert.match(build, /VERSION="0\.4\.8"/);
});
