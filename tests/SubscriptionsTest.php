...
class FakeWpdb {
    ...
    public function get_var($prepared){
        if (is_array($prepared) && ($prepared['type'] ?? '') === 'exists') return $this->table_exists?'wp_lousy_outages_subs':null;
        $q = is_array($prepared) ? strtolower((string)($prepared['q'] ?? '')) : strtolower((string)$prepared);
        if (str_contains($q, 'select count(*)')) {
            if (str_contains($q, 'status in')) return count(array_filter($this->rows, fn($r)=>in_array($r['status']??'', ['subscribed','confirmed'], true)));
            if (str_contains($q, 'status =')) return count(array_filter($this->rows, fn($r)=>($r['status']??'')==='pending'));
            if (str_contains($q, 'realtime_alerts')) return count(array_filter($this->rows, fn($r)=>!empty($r['realtime_alerts'])));
            if (str_contains($q, 'daily_digest')) return count(array_filter($this->rows, fn($r)=>!empty($r['daily_digest'])));
            if (str_contains($q, 'newsletter')) return count(array_filter($this->rows, fn($r)=>!empty($r['newsletter'])));
...
$tests['stats_zero_when_table_missing']=function(){ global $wpdb; $wpdb=new \FakeWpdb(); $wpdb->table_exists=false; $s=Subscriptions::stats(); if($s!==['total'=>0,'confirmed'=>0,'pending'=>0,'realtime'=>0,'digest'=>0,'newsletter'=>0]) throw new \RuntimeException('expected zeros'); };
