...
function lousy_outages_admin_dashboard_page() { $stats = class_exists('SuzyEaston\LousyOutages\Subscriptions') ? SuzyEaston\LousyOutages\Subscriptions::stats() : []; $providers = Providers::enabled(); $community = class_exists('SuzyEaston\LousyOutages\UserReports') ? SuzyEaston\LousyOutages\UserReports::recent(60,100) : []; $external = class_exists('SuzyEaston\LousyOutages\ExternalSignals') ? SuzyEaston\LousyOutages\ExternalSignals::recent(60,100) : []; $fused = get_option('lousy_outages_signal_engine_last_fused', []); if ( ! is_array($fused) ) { $fused = []; } echo '<div class="wrap"><h1>Lousy Outages Dashboard</h1><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;max-width:1120px;">'; $cards=['Monitored Providers'=>count($providers),'Active Subscribers'=>(int)($stats['confirmed']??0),'Community Reports Last Hour'=>is_array($community)?count($community):0,'External Signals Last Hour'=>is_array($external)?count($external):0,'Fused Signals'=>count($fused),'Last Poll'=>(string)get_option('lousy_outages_last_poll','—'),'Last External Collection'=>(string)get_option('lousy_outages_last_external_collection','—')]; foreach($cards as $label=>$value){ echo '<div style="border:1px solid #ccd0d4;background:#fff;padding:12px;"><strong>'.esc_html($label).'</strong><div style="font-size:18px;margin-top:8px;">'.esc_html((string)$value).'</div></div>'; } echo '</div><p style="margin-top:16px;"><a class="button" href="'.esc_url(home_url('/lousy-outages/')).'">View Public Page</a> <a class="button" href="'.esc_url(wp_nonce_url(admin_url('admin-post.php?action=lousy_outages_poll_now'),'lousy_outages_poll_now')).'">Poll Now</a> <a class="button" href="'.esc_url(wp_nonce_url(admin_url('admin-post.php?action=lousy_outages_collect_signals_now'),'lousy_outages_collect_signals_now')).'">Collect External Signals Now</a> <a class="button button-primary" href="'.esc_url(admin_url('admin.php?page=lousy-outages-settings')).'">Settings</a></p></div>'; }
