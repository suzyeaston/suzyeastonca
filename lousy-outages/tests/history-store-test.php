...
$canonical = get_option('lo_event_log_v2', []);
assert_true(count($canonical) === $report['after_dedupe'], 'canonical option holds the deduped event set');
$thirty = array_filter($canonical, fn($e) => (int)$e['first_seen'] >= $now - 30 * DAY_IN_SECONDS);
assert_true(count($thirty) < count($canonical), '30-day filter hides older records without deleting them');
$one_year = array_filter($canonical, fn($e) => (int)$e['first_seen'] >= $now - 365 * DAY_IN_SECONDS);
assert_true(count($one_year) >= count($thirty), 'one-year request returns retained older events');
$awsApril = array_values(array_filter($canonical, fn($e) => $e['provider'] === 'aws' && str_contains($e['title'], 'April')));
