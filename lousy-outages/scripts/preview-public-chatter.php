...
echo "Accepted rows: " . count($rows) . "\n";
foreach ($rows as $r) {
    echo "[ACCEPT] " . ($r['provider_name'] ?? '') . " | " . ($r['title'] ?? '') . " | " . ($r['evidence_quote'] ?? '') . "\n";
}
echo "\nCandidate preview (accepted/rejected):\n";
foreach ($preview as $p) {
    $title = substr((string)($p['title'] ?? ''),0,120);
    $quote = substr((string)($p['quote'] ?? ''),0,160);
    $reason = (string)($p['reject_reason'] ?? '');
    echo "- {$reason} | {$title} | {$quote}\n";
}
